...
tcc = np.array(f.variables['clt'][:60])
...
ps = np.array(f.variables['ps'][:60]) #in hPa
f.close()
...
lw = np.array(f.variables['clw'][:60])
...
with h5py.File('2001_2005_giss_e21g.h5', 'w') as p:
