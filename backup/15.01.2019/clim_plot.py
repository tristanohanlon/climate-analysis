...
def liq_temp_plot_models( models_to_graph, cmip5_range = True, cmip6_range = True ):
    ...
    ax1.set_ylabel( 'Mean Cloud Liquid Water Mass Fraction in Air (g/kg)' )
    ax2.set_ylabel( 'Mean Cloud Liquid Water Mass Fraction in Air (g/kg)' )
    ax2.set_xlabel( 'Temperature (K)' )
    ax1.axvline(x=273, label = '273K', color = 'black', linestyle='--')
    ax2.axvline(x=273, label = '273K', color = 'black', linestyle='--')
...
def liqc_temp_plot_models( models_to_graph, cmip5_range = True, cmip6_range = True ):

    cmip5models_g = [ model.data['clwc_t_g'] for name, model in models.items() if name.startswith('CMIP5') ]
    cmip6models_g = [ model.data['clwc_t_g'] for name, model in models.items() if name.startswith('CMIP6') ]
    cmip5models_so = [ model.data['clwc_t_so'] for name, model in models.items() if name.startswith('CMIP5') ]
    cmip6models_so = [ model.data['clwc_t_so'] for name, model in models.items() if name.startswith('CMIP6') ]
    ...
    fig, ( ax1, ax2 ) = plt.subplots(2,1, figsize=(15, 7))
    for model, col in zip( models_to_graph, colours ):
        ax1.plot( constants.ta, model.data['clwc_t_g'],  col, label=model.name )
        ax2.plot( constants.ta, model.data['clwc_t_so'], col, label=model.name )
...
def satellite_contours( models_to_graph, zero_lines ):
    ...
    for count, (model, zero_line ) in enumerate( zip( models_to_graph, zero_lines ) ):
        prime = ax[ count ].contourf( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt[:constants.liq_alt_confine], model.data['full_clw_frac_alt_lat'][:constants.liq_alt_confine,constants.lat_confine_1:constants.lat_confine_2], vmin=0, vmax=0.36, cmap='coolwarm' )
        temp = ax[ count ].contour( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt[:constants.liq_alt_confine], ( model.data[ 'full_ta_alt_lat' ][:constants.liq_alt_confine,constants.lat_confine_1:constants.lat_confine_2] - 273.15 ), colors='white' )
    ...
    cbar.set_label('Mean Cloud Liquid Water Content (g/m^3)')    
    cbar.set_clim(0, 0.36)
...
def model_clw_frac_contours( models_to_graph, zero_lines ):
    ...
    fig, ax = plt.subplots( nrows=1, ncols=len( models_to_graph ), figsize=(15, 5) )
    for count, (model, zero_line ) in enumerate( zip( models_to_graph, zero_lines ) ):
        prime = ax[ count ].contourf( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.liq_alt, model.data['clw_frac_alt_lat'][:,constants.lat_confine_1:constants.lat_confine_2], vmin=0, vmax=0.36, cmap='coolwarm' )
        temp = ax[ count ].contour( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.liq_alt, ( model.data[ 'ta_alt_lat' ][:,constants.lat_confine_1:constants.lat_confine_2] - 273.15 ), colors='white' )
        temp.collections[ zero_line ].set_linewidth( 3 )
        temp.collections[ zero_line ].set_color( 'white' )
        ax[ count ].clabel( temp, inline=1, fontsize=10 )
...
def region_plot( models_to_graph, quantity_to_graph ):
    ...
    plt.savefig( location + '/Images/' + 'regional' + '_' +  all_model_names + '_' + quantity_to_graph + ".svg", format="svg", bbox_inches='tight')
    plt.show()
