...
cosp_models = {
    ...
    'CMIP5-AMIP-GISS-E2R' : Model( location + 'climate-analysis/reduced_data/' + 'COSP_' + constants.date_cmip5 + '_CMIP5-AMIP-GISS-E2R.h5', 'CMIP5-AMIP-GISS-E2R-COSP' ),
    'CMIP5-AMIP-IPSL-CM5A-LR' : Model( location + 'climate-analysis/reduced_data/' + 'COSP_' + constants.date_cmip5 + '_CMIP5-AMIP-IPSL-CM5A-LR.h5', 'CMIP5-AMIP-IPSL-CM5A-LR-COSP' ),
    ...
    'CMIP6-AMIP-IPSL-CM6A-LR' : Model( location + 'climate-analysis/reduced_data/' + 'COSP_' + constants.date_cmip6 + '_CMIP6-AMIP-IPSL-CM6A-LR.h5', 'CMIP6-AMIP-IPSL-CM6A-LR-COSP' ),
}
...
def latitude_plot_models( models_to_graph, quantity_to_graph, cmip5_range = True, cmip6_range = True ):
    ...
    if cmip5_range == True:
        ...
        cmip5models = [ model.data[quantity_to_graph] for name, model in cosp_models.items() if name.startswith('CMIP5') ]
        cmip5_max = np.maximum.reduce( cmip5models )
        cmip5_min = np.minimum.reduce( cmip5models )

        ax.fill_between( constants.lat[constants.lat_confine_1:constants.lat_confine_2], cmip5_min[constants.lat_confine_1:constants.lat_confine_2], cmip5_max[constants.lat_confine_1:constants.lat_confine_2], facecolor='red', alpha=0.3,
                    label='CMIP5 Model Range' )
        
    if cmip6_range == True:
        cmip6models = [ model.data[quantity_to_graph] for name, model in cosp_models.items() if name.startswith('CMIP6') ]
        cmip6_max = np.maximum.reduce( cmip6models )
        cmip6_min = np.minimum.reduce( cmip6models )

        ax.fill_between( constants.lat[constants.lat_confine_1:constants.lat_confine_2], cmip6_min[constants.lat_confine_1:constants.lat_confine_2], cmip6_max[constants.lat_confine_1:constants.lat_confine_2], facecolor='blue', alpha=0.3,
...
def altitude_plot_models( models_to_graph, quantity_to_graph, cmip5_range = True, cmip6_range = True ):
    ...
    if cmip5_range == True:
        ...
        if quantity_to_graph == 'clw' or quantity_to_graph == 'clwc':
            ax1.fill_betweenx( constants.liq_alt, cmip5_g_min*1000, cmip5_g_max*1000, facecolor='red', alpha=0.3,
                        label='CMIP5 Model Range' )
            ...
            ax1.fill_betweenx( constants.alt, cmip5_g_min, cmip5_g_max, facecolor='red', alpha=0.3,
                        label='CMIP5 Model Range' )
            ax2.fill_betweenx( constants.alt, cmip5_so_min, cmip5_so_max, facecolor='red', alpha=0.3,
                        label='CMIP5 Model Range' )
    ...
    if cmip6_range == True:
        ...
        if quantity_to_graph == 'clw' or quantity_to_graph == 'clwc':
            ...
            ax1.fill_betweenx( constants.alt, cmip6_g_min, cmip6_g_max, facecolor='blue', alpha=0.3,
                        label='CMIP6 Model Range' )    
            ax2.fill_betweenx( constants.alt, cmip6_so_min, cmip6_so_max, facecolor='blue', alpha=0.3,
                        label='CMIP6 Model Range' )
    ...
    ax2.legend(loc='upper center', bbox_to_anchor=(1.65, 1.0));
...
def cli_frac_temp_plot_models( models_to_graph, quantity_to_graph, cmip5_range = True, cmip6_range = True ):
    ...
    all_model_names = '_'.join( [ model.name for model in models_to_graph ])
    fig.tight_layout()
    plt.savefig( location + '/Images/' + 'cli_frac_t' + '_' +  all_model_names + ".svg", format="svg", bbox_inches='tight' )
    plt.show()

############################################################################### contour plots

def cl_contour( models_to_graph, zero_lines ):
    assert( len( models_to_graph ) == len( zero_lines ) )

    fig, ax = plt.subplots( )
    for count, (model, zero_line ) in enumerate( zip( models_to_graph, zero_lines ) ):
        prime = ax.contourf( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt, model.data['cl_alt_lat'][:,constants.lat_confine_1:constants.lat_confine_2], vmin=0, vmax=0.4, cmap='coolwarm' )
        temp = ax.contour( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt, ( model.data[ 'full_ta_alt_lat' ][:,constants.lat_confine_1:constants.lat_confine_2] - 273.15 ), colors='white' )
    ...
    cbar.set_label('Mean Cloud Fraction')    
    cbar.set_clim(0, 0.4)

    all_model_names = '_'.join( [ model.name for model in models_to_graph ])

    plt.savefig( location + '/Images/' + 'cl_contour' + '_' +  all_model_names + ".svg", format="svg", bbox_inches='tight')
    plt.show()


def clw_frac_contour( models_to_graph, zero_lines ):
    assert( len( models_to_graph ) == len( zero_lines ) )
    ...
    plt.savefig( location + '/Images/' + 'clw_frac_contour' + '_' +  model.name + ".svg", format="svg", bbox_inches='tight')
    plt.show()

   
def cli_frac_contour( models_to_graph, zero_lines ):
    assert( len( models_to_graph ) == len( zero_lines ) )

    fig, ax = plt.subplots( )
    for count, (model, zero_line ) in enumerate( zip( models_to_graph, zero_lines ) ):
        prime = ax.contourf( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt, model.data['cli_frac_alt_lat'][:,constants.lat_confine_1:constants.lat_confine_2], vmin=0, vmax=0.25, cmap='coolwarm' )
        temp = ax.contour( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt, ( model.data[ 'full_ta_alt_lat' ][:,constants.lat_confine_1:constants.lat_confine_2] - 273.15 ), colors='white' )
        temp.collections[ zero_line ].set_linewidth( 3 )
        temp.collections[ zero_line ].set_color( 'white' )
        ax.clabel( temp, inline=1, fontsize=10 )
    ...
    plt.savefig( location + '/Images/' + 'cli_frac_contour' + '_' +  all_model_names + ".svg", format="svg", bbox_inches='tight')
    plt.show()
...
def model_cl_contours( models_to_graph, zero_lines ):
    ...
    fig, ax = plt.subplots( nrows=1, ncols=len( models_to_graph ), figsize=(15, 5) )
    for count, (model, zero_line ) in enumerate( zip( models_to_graph, zero_lines ) ):
        prime = ax[ count ].contourf( constants.lat[constants.lat_confine_1:constants.lat_confine_2], constants.alt, model.data['cl_alt_lat'][:,constants.lat_confine_1:constants.lat_confine_2], vmin=0, vmax=0.4, cmap='coolwarm' )
...
def calipso_region_plot( models_to_graph, quantity_to_graph ):
    ax = plt.axes( projection=ccrs.Mollweide(  central_longitude=180 ) )
    ax.coastlines()
    for model in  models_to_graph:

        prime = ax.contourf( constants.lon, constants.lat, np.roll(model.data[ quantity_to_graph + '_lat_lon' ], 179), transform=ccrs.PlateCarree(), cmap='coolwarm' )
        ax.set_title( model.name )
...
def calipso_region_bias_plot( models_to_graph, quantity_to_graph ):
    ax = plt.axes( projection=ccrs.PlateCarree(  central_longitude=180 ) )
    ax.coastlines()
    for model in  models_to_graph:
        # if model == 'CALIPSO':
        ...
        prime = ax.contourf( constants.lon, constants.lat, ( np.roll(models['CALIPSO'].data[ quantity_to_graph + '_lat_lon' ], 179) - model.data[ quantity_to_graph + '_lat_lon' ]), transform=ccrs.PlateCarree(), cmap='coolwarm' )
        ax.set_title( model.name + ' Bias (Observed - Simulated)' )

        cbar = plt.colorbar(prime, orientation='horizontal')

        cbar.set_label( quantity[quantity_to_graph] + ' Bias (Observed - Simulated)' )

        plt.savefig( location + '/Images/' + 'regional' + '_' +  quantity_to_graph + '_' + 'bias' +  model.name + ".svg", format="svg", bbox_inches='tight')
        plt.show()
