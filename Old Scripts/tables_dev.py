...
"""
def LongSITable():
    with open('86km.csv', mode='a') as output_file:
        writer = csv.writer( output_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL )
        writer.writerows('alt(km)','sigma','delta','theta','temp(K)','press(Pa)','density', 'c','visc','k.visc','ratio') 
        for i in range(-1,44):
            altKm=2*i
            (sigma, delta, theta) = Atmosphere(altKm)
            writer.writerows("%4i %8.4E %8.4E %6.4f" %
                (2*i, sigma,delta,theta))
            temp=TZERO*theta
            pressure=PZERO*delta
            density=RHOZERO*sigma
            asound=AZERO*math.sqrt(theta)
            writer.writerows("%6.1f %8.3E %8.3E %5.1f" %
                (temp,pressure,density,asound))
            viscosity=MetricViscosity(theta)
            kinematicViscosity=viscosity/density
            writer.writerows("%6.2f %8.2E\n" %
                (1.0E6*viscosity, kinematicViscosity))

    writer.close()

def ShortSITable():
    with open('20km.csv', mode='a') as output_file:
        writer = csv.writer( output_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL )
        writer.writerows('alt(km)','sigma','delta','theta','temp(K)','press(Pa)','density', 'c','visc','k.visc','ratio') 
    ...
    for i in range(-1, 41):
        altKm=0.5*i
        (sigma, delta, theta) = SimpleAtmosphere(altKm)
        writer.writerows("%4.1f %6.4f %6.4f %6.4f" %
            (altKm, sigma, delta, theta))
        temp=TZERO*theta
        pressure=PZERO*delta
        density=RHOZERO*sigma
        asound=AZERO*math.sqrt(theta)
        writer.writerows("%6.1f %6.0f %5.3f %5.1f" %
            (temp, pressure,density,asound))
        viscosity=MetricViscosity(theta)
        kinematicViscosity=viscosity/density
        vratio=asound/kinematicViscosity
        writer.writerows("%6.2f %8.2E %5.2f\n" %
            (1.0E6*viscosity, kinematicViscosity, 1.0E-6*vratio))
