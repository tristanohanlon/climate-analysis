...
def radiation(start, end, start_dt, end_dt, location, models, label, lat_bnd_1, lat_bnd_2, save_outputs, liquid_r, ice_r, set_alb_insol ):
    ...
    #-------------------------- Surface radiative flux data --------------------------#
        ...
        print (model, ':', 'SW CRE', ':', (constants.globalMean(rsutcs, T_lat) - constants.globalMean(rsut, T_lat)))
        print (model, ':', 'LW CRE', ':', (constants.globalMean(rlutcs, T_lat) - constants.globalMean(rlut, T_lat)))
        ...
        model_SW_CREglobals[name] = (constants.globalMean(rsutcs, T_lat) - constants.globalMean(rsut, T_lat))
        model_LW_CREglobals[name] = (constants.globalMean(rlutcs, T_lat) - constants.globalMean(rlut, T_lat))
    ...
    SW_CRE_mean = ensemble_mean(model_SW_CREglobals)
    LW_CRE_mean = ensemble_mean(model_LW_CREglobals)
    ...
    # toggle tuned values, false will use the ensemble mean
    if set_alb_insol == True:
        albedomean = np.zeros_like(state.Ts) + 0.13
        INSOLATIONmean = np.zeros_like(state.Ts) + 266
    ...
    print('===RRTMG CRE===')
    print('SW CRE:')
    print(constants.globalMean(control_rsutcs, lat) - constants.globalMean(control_rsut, lat))
    print('LW CRE:')
    print(constants.globalMean(control_rlutcs, lat) - constants.globalMean(control_rlut, lat))
    ...
    for model_name, model_cl_value in model_CLglobals.items():
        for i in range(lev.size):
            cl_rad = RRTMG(state=state, 
                        albedo= albedomean, #np.zeros_like(state.Ts) + ideal_albedo_mean,
                        insolation = INSOLATIONmean, #np.zeros_like(state.Ts) + ideal_insolation_mean,
                        absorber_vmr=absorbermean,
                        specific_humidity=np.transpose(SHmean),
