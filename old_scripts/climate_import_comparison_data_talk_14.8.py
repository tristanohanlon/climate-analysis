...
p = h5py.File('2001_2005_cesm1_cam5.h5', 'r')
...
cam5_tcc_lat_g = p['tcc'][:] # 0-1
cam5_tclw_lat_g = p['tclw'][:] #kgm^-2
cam5_tciw_lat_g = p['tciw'][:] #kgm^-2


#---CESM1-CAM5-AMIP Global Latitude Phase Fractions---#

# 0-1 derived from fraction of mean liquid and ice water content at specific latitude * total cloud fraction at the latitude
cam5_tclw_frac_lat_g = p['tclw_frac'][:] 
cam5_tciw_frac_lat_g = p['tciw_frac'][:]
...
cam5_tcc_alt_lat = p['cf_alt_lat'][:] 
cam5_tclw_alt_lat = p['lw_alt_lat'][:] #kg/kg
cam5_tciw_alt_lat = p['iw_alt_lat'][:] #kg/kg
cam5_temp_alt_lat = p['temp_alt_lat'][:] #kg/kg
cam5_lat = p['lat'][:]
cam5_lat = np.hstack(cam5_lat)
cam5_alt = p['alt'][:]
cam5_alt = np.hstack(cam5_alt)
cam5_alt_temp = p['alt_temp'][:]
...
cam5_tcc_alt_g = p['cf'][:] # 0-1
cam5_tclw_alt_g = p['lw'][:] #kg/kg
cam5_tciw_alt_g = p['iw'][:] #kg/kg
cam5_temp_alt_g = p['temp'][:] #K
cam5_plevel_alt_g = p['pressure'][:] #hPa

cam5_tcc_temp_g = p['cf_t'][:] # 0-1
cam5_tclw_temp_g = p['lw_t'][:] #kg/kg
cam5_tciw_temp_g = p['iw_t'][:] #kg/kg

cam5_tclw_frac_temp_g = p['lw_frac_t'][:]
cam5_tciw_frac_temp_g = p['iw_frac_t'][:]


#---CESM1-CAM5-AMIP Phase Profile Fractions---#

cam5_tclw_frac_alt_g = p['lw_frac'][:]
cam5_tciw_frac_alt_g = p['iw_frac'][:]
cam5_tclw_frac_alt_so = p['lw_frac_so'][:]
cam5_tciw_frac_alt_so = p['iw_frac_so'][:]


#---CESM1-CAM5-AMIP Southern Ocean Profile---#

cam5_tcc_alt_so = p['cf_so'][:] # 0-1
cam5_tclw_alt_so = p['lw_so'][:] #kg/kg
cam5_tciw_alt_so = p['iw_so'][:] #kg/kg

cam5_tcc_temp_so = p['cf_t_so'][:] # 0-1
cam5_tclw_temp_so = p['lw_t_so'][:] #kg/kg
cam5_tciw_temp_so = p['iw_t_so'][:] #kg/kg

cam5_tclw_frac_temp_so = p['lw_frac_t_so'][:]
cam5_tciw_frac_temp_so = p['lw_frac_t_so'][:]
...
os.chdir('c:/Users/tristan/University/University/MSc/Models/Images/Talk 14.8')
...
ax1.plot(calipso_tclw_frac_temp_g[17:34,0],(calipso_tclw_frac_temp_g[17:34,1]/calipso_tcc_temp_g[17:34,1]), '-b', label='CALIPSO-GOCCP')
...
ax1.plot(cam5_tclw_frac_temp_g[:13,0],(cam5_tclw_frac_temp_g[:13,1]/cam5_tcc_temp_g[:13,1]), '--m', label='CMIP5-CESM1-CAM5-AMIP')
ax1.plot(gfdl4_tclw_frac_temp_g[:20,0],(gfdl4_tclw_frac_temp_g[:20,1]/gfdl4_tcc_temp_g[:20,1]), '-g', label='CMIP6-GFDL-AM4-AMIP')
ax1.plot(cam6_tclw_frac_temp_g[16:,0],(cam6_tclw_frac_temp_g[16:,1]/cam6_tcc_temp_g[16:,1]), '-m', label='CMIP6-CESM2-CAM6-AMIP')

ax2.plot(calipso_tclw_frac_temp_so[17:34,0],(calipso_tclw_frac_temp_so[17:34,1]/calipso_tcc_temp_so[17:34,1]), '-b', label='CALIPSO-GOCCP')
ax2.plot(ecmwf_tclw_frac_temp_so[18:,0],(ecmwf_tclw_frac_temp_so[18:,1]/ecmwf_tcc_temp_so[18:,1]), '-k', label='ECMWF-ERA5')
ax2.plot(gfdl_hiram_tclw_frac_temp_so[:19,0],(gfdl_hiram_tclw_frac_temp_so[:19,1]/gfdl_hiram_tcc_temp_so[:19,1]), '--g', label='CMIP5-GFDL-HIRAM-AMIP')
ax2.plot(cam5_tclw_frac_temp_so[:13,0],(cam5_tclw_frac_temp_so[:13,1]/cam5_tcc_temp_so[:13,1]), '--m', label='CMIP5-CESM1-CAM5-AMIP')
ax2.plot(gfdl4_tclw_frac_temp_so[:20,0],(gfdl4_tclw_frac_temp_so[:20,1]/gfdl4_tcc_temp_so[:20,1]), '-g', label='CMIP6-GFDL-AM4-AMIP')
ax2.plot(cam6_tclw_frac_temp_so[16:,0],(cam6_tclw_frac_temp_so[16:,1]/cam6_tcc_temp_so[16:,1]), '-m', label='CMIP6-CESM2-CAM6-AMIP')
...
ax1.set_ylabel('Cloud Liquid Fraction')
ax2.set_xlabel('Temperature (K)')
ax2.set_ylabel('Cloud Liquid Fraction')

ax1.grid(True)
ax2.grid(True)

plt.savefig("liquid_T.pdf", format="pdf", bbox_inches='tight')
plt.show()
"""
...
ax1.plot(ceres_tcc_lat_g[:,0],ceres_tcc_lat_g[:,1], '-k', label='CERES')
...
ax1.plot(gfdl4_tcc_lat_g[:,0],gfdl4_tcc_lat_g[:,1], '-g', label='CMIP6-GFDL-AM4-AMIP')
ax1.plot(cam6_tcc_lat_g[:,0],cam6_tcc_lat_g[:,1], '-m', label='CMIP6-CESM2.1-CAM6-AMIP')


#ax1.fill_between(cam5_tcc_lat_g[:,0], cam5_tcc_lat_g[:,1], cccm_tcc_lat_g_enhanced[:,1], facecolor='yellow', alpha=0.5,
#                label='1 sigma range')
...
ax1.plot(cccm_tcc_alt_g[4:92,1],cccm_tcc_alt_g[4:92,0], '-r', label='A-Train - CCCM')
ax1.plot(calipso_tcc_alt_g[:,1],calipso_tcc_alt_g[:,0], '-b', label='CALIPSO-GOCCP')
...
ax1.plot(cam5_tcc_alt_g[:17,1],cam5_tcc_alt_g[:17,0], '--m', label='CMIP5-CESM1-CAM5-AMIP')
ax1.plot(gfdl4_tcc_alt_g[:23,1],gfdl4_tcc_alt_g[:23,0], '-g', label='CMIP6-GFDL-AM4-AMIP')
ax1.plot(cam6_tcc_alt_g[11:,1],cam6_tcc_alt_g[11:,0], '-m', label='CMIP6-CESM2.1-CAM6-AMIP')

ax2.plot(cccm_tcc_alt_so[4:92,1],cccm_tcc_alt_so[4:92,0], '-r', label='CCCM')
ax2.plot(calipso_tcc_alt_so[:,1],calipso_tcc_alt_so[:,0], '-b', label='CALIPSO-GOCCP')
ax2.plot(ecmwf_tcc_alt_so[9:,1],ecmwf_tcc_alt_so[9:,0], '-k', label='ECMWF-ERA5')

ax2.plot(gfdl_hiram_tcc_alt_so[:23,1],gfdl_hiram_tcc_alt_so[:23,0], '--g', label='CMIP5-GFDL-HIRAM-AMIP')
ax2.plot(cam5_tcc_alt_so[:17,1],cam5_tcc_alt_so[:17,0], '--m', label='CMIP5-CESM1-CAM5-AMIP')
ax2.plot(gfdl4_tcc_alt_so[:23,1],gfdl4_tcc_alt_so[:23,0], '-g', label='CMIP6-GFDL-AM4-AMIP')
ax2.plot(cam6_tcc_alt_so[12:,1],cam6_tcc_alt_so[12:,0], '-m', label='CMIP6-CESM2.1-CAM6-AMIP')

ax2.legend(loc='center', bbox_to_anchor=(1.8, 0.8));

ax1.set_ylabel('Altitude (km)')
ax1.set_xlabel('Global Cloud Fraction')
ax2.set_xlabel('Southern Ocean Cloud Fraction')
...
plt.savefig("tcc_alt.pdf", format="pdf", bbox_inches='tight')
...
ax1.plot(calipso_tclw_frac_alt_g[:20,1],calipso_tclw_frac_alt_g[:20,0], '-b', label='CALIPSO-GOCCP')
...
ax1.plot(cam5_tclw_frac_alt_g[:13,1],cam5_tclw_frac_alt_g[:13,0], '--m', label='CMIP5-CESM1-CAM5-AMIP')
ax1.plot(gfdl4_tclw_frac_alt_g[:18,1],gfdl4_tclw_frac_alt_g[:18,0], '-g', label='CMIP6-GFDL-AM4-AMIP')
ax1.plot(cam6_tclw_frac_alt_g[17:,1],cam6_tclw_frac_alt_g[17:,0], '-m', label='CMIP6-CESM2.1-CAM6-AMIP')

ax2.plot(cccm_tclw_frac_alt_so[4:50,1],cccm_tclw_frac_alt_so[4:50,0], '-r', label='CCCM')
ax2.plot(calipso_tclw_frac_alt_so[:20,1],calipso_tclw_frac_alt_so[:20,0], '-b', label='CALIPSO-GOCCP')
ax2.plot(ecmwf_tclw_frac_alt_so[18:,1],ecmwf_tclw_frac_alt_so[18:,0], '-k', label='ECMWF-ERA5')

ax2.plot(gfdl_hiram_tclw_frac_alt_so[:18,1],gfdl_hiram_tclw_frac_alt_so[:18,0], '--g', label='CMIP5-GFDL-HIRAM-AMIP')
ax2.plot(cam5_tclw_frac_alt_so[:13,1],cam5_tclw_frac_alt_so[:13,0], '--m', label='CMIP5-CESM1-CAM5-AMIP')
ax2.plot(gfdl4_tclw_frac_alt_so[:18,1],gfdl4_tclw_frac_alt_so[:18,0], '-g', label='CMIP6-GFDL-AM4-AMIP')
ax2.plot(cam6_tclw_frac_alt_so[17:,1],cam6_tclw_frac_alt_so[17:,0], '-m', label='CMIP6-CESM2.1-CAM6-AMIP')

ax2.legend(loc='center', bbox_to_anchor=(1.7, 0.8));

ax1.set_ylabel('Altitude (km)')
ax1.set_xlabel('Global Cloud Liquid Water Fraction')
ax2.set_xlabel('Southern Ocean Cloud Liquid Water Fraction')
...
ax[0, 1].contourf(ecmwf_lat, ecmwf_alt[19:], ecmwf_tclw_frac_alt_lat[19:], vmin=0, vmax=0.4)
...
ax[0, 0].contourf(calipso_lat, calipso_alt[0:16], calipso_tclw_frac_alt_lat[0:16], vmin=0, vmax=0.4)
...
ax[1, 0].contourf(gfdl_hiram_lat, gfdl_hiram_alt[0:17], gfdl_hiram_tclw_frac_alt_lat[0:17], vmin=0, vmax=0.4)
...
cont = ax[1, 1].contourf(gfdl4_lat, gfdl4_alt[0:18], gfdl4_tclw_frac_alt_lat[0:18], vmin=0, vmax=0.4)
...
ax[2, 0].set_title('e) CMIP5-CESM1-CAM5')
...
ax[2, 1].set_title('f) CMIP6-CESM2.1-CAM6')
