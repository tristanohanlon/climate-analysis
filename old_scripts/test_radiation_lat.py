...
with xr.open_dataset(constants.variable_to_filename( 'cl' ), decode_times=False) as cl:
    ...
    CLglobal = (cl.cl * weight).mean(dim=('lon','time')) / 100
    a = cl.ap
    b = cl.b
...
interpolated = interpolate.interp2d( CLglobal.lat, np.flipud(Tglobal.plev[:18]), np.flipud(Tglobal[:18]))
Tinterp = interpolated( CLglobal.lat, lev )
