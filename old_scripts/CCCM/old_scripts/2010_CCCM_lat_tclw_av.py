...
This will create a datasset of global average specific cloud liquid water content (kg/kg) with latitude.
...
#computing the total specific cloud liquid water cloud content (LWP) kg/kg

a_tclw = tclw / air_density # divide by air density at each altitude

# Average the ice water content over latitude and longitude for each altitude level 

s_tclw = np.nanmean(a_tclw, axis=1)

# Join the two lists as if they were two columns side by side, into a list of two elements each
combined = np.vstack((lat, s_tclw)).T
#print ("combined")
#print (combined)
