...
tc = np.mean(etcc, axis=0) #average over time
tcc = np.mean(tc, axis=1) #average over time
ecmwf_tcc_lat = np.vstack((elat, tcc)).T

plt.figure()
plt.plot(ecmwf_tcc_lat[:,0],ecmwf_tcc_lat[:,1])
plt.show()