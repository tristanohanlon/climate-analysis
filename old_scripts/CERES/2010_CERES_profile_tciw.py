...
This will extract the ice water cloud fraction profile against altitude. 
The global ice water cloud fraction is already averaged over all longitude and latitude at each altitude layer.
[:,0] = altitude
[:,1] = ice water cloud fraction
"""
...
iw=[] # create a blank array to add ice water cloud amount data
...
for filename in os.listdir(): 
    ...
    alt = alt+f.select('cld_eff_hgt_glob').get().tolist()[:-1]
    
    # Get the ice water cloud fraction data as a list, excluding the last value which is the total for all altitudes
    iw = iw+f.select('cld_amount_ice_glob').get()[:-1].tolist()

if len(alt) != len(iw):
    exit('Invalid sizes of alt and iw data')


# Join the two lists as if they were two columns side by side, into a list of two elements each
alt = np.array(alt)
iw = np.array(iw)
combined = np.vstack((alt, iw)).T
...
# Averages of (alt,ice water cloud fraction) array
ceres_tciw_alt = np.empty((unique.size,2),dtype=float)

# Current subtotal of current alt
subtotal = 0.0
# Current number of ice water cloud fraction entries in subtotal
number = 0
# Set the current alt to false
current_alt = None

# Iterate through all of the (alt,ice water cloud fraction) elements and subtotal the same alt values
i = 0
for item in combined:
    ...
    if item[0] != current_alt:
        ...
        """
        # Append the average
        ceres_tciw_alt[i] = [current_alt, average]
        # Reset the subtotal
        subtotal = 0.0
        number = 0
...
"""
print ("averages")
# Iterate through all of the (alt,ice water cloud fraction) elements
